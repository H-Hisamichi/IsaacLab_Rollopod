...
class RollopodRewards(RewardsCfg):
    ...
    track_lin_vel_xy_w_exp = RewTerm(
        func=mdp.track_lin_vel_xy_w_exp, weight=0.0, params={"command_name": "base_velocity", "std": math.sqrt(2.0)}
    )
    ...
    track_rolling_ang_vel_exp = RewTerm(
        func=mdp.track_rolling_ang_vel_exp, weight=1.0, params={"command_name": "base_velocity", "std": math.sqrt(2.0)}
    )
    track_rolling_ang_vel_exp_fine_grained = RewTerm(
        func=mdp.track_rolling_ang_vel_exp, weight=1.0, params={"command_name": "base_velocity", "std": math.sqrt(0.2)}
    )
    # -- optional penalties
    ...
    rolling_slip_penalty = RewTerm(
        func=mdp.rolling_slip_penalty_v2, weight=0.0, params={ "scale": 0.6, "rolling_radius": 0.43}
    )

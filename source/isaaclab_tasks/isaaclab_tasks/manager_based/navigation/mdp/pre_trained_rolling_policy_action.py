...
class PreTrainedRollingPolicyAction(ActionTerm):
    ...
    cfg: PreTrainedRollingPolicyActionCfg
    """The configuration of the action term."""

    def __init__(self, cfg: PreTrainedRollingPolicyActionCfg, env: ManagerBasedRLEnv) -> None:
    ...
    def _debug_vis_callback(self, event):
        ...
        direction = self.raw_actions[:, :2]
        #direction_w = math_utils.quat_apply(self.robot.data.root_quat_w, direction)
        wheel_radius = 0.33
        linear_speed = torch.abs(self.raw_actions[:, -1]) * wheel_radius
        goal_xyz_velocity = direction * linear_speed.unsqueeze(1)
        vel_des_arrow_scale, vel_des_arrow_quat = self._resolve_xyz_direction_to_arrow(self.raw_actions)
    ...
    def _resolve_xyz_direction_to_arrow(self, _raw_actions: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        """Converts the XY base velocity command to arrow direction rotation."""
        # obtain default scale of the marker
        default_scale = self.base_vel_goal_visualizer.cfg.markers["arrow"].scale
        # arrow-scale
        direction = _raw_actions[:, :2]
        wheel_radius = 0.33
        linear_speed = torch.abs(_raw_actions[:, -1]) * wheel_radius
        arrow_scale = torch.tensor(default_scale, device=self.device).repeat(direction.shape[0], 1)
        arrow_scale[:, 0] *= torch.linalg.norm(direction, dim=1) * linear_speed * 3.0
        # arrow-direction
        yaw = torch.atan2(direction[:, 1], direction[:, 0])
        pitch = torch.asin(direction[:, 2])
        roll = torch.zeros_like(yaw)
        arrow_quat = math_utils.quat_from_euler_xyz(roll, pitch, yaw)
        # convert everything back from base to world frame
        base_quat_w = self.robot.data.root_quat_w
        arrow_quat = math_utils.quat_mul(base_quat_w, arrow_quat)
...
@configclass
class PreTrainedRollingPolicyActionCfg(ActionTermCfg):
    """Configuration for pre-trained policy action term.

    See :class:`PreTrainedPolicyAction` for more details.
    """

    class_type: type[ActionTerm] = PreTrainedRollingPolicyAction

...
class JumpingCommand(CommandTerm):
    ...
    def _update_metrics(self):
        ...
        self.metrics["error_pos"] = torch.abs(self.pos_command_w[:, -1] - self.robot.data.root_lin_vel_b[:, -1])

    def _resample_command(self, env_ids: Sequence[int]):
        # obtain env origins for the environments
        #self.pos_command_w[env_ids] = self._env.scene.env_origins[env_ids]
        self.pos_command_w[env_ids] = 0.0
        ...
        self.pos_command_w[env_ids, 2] += r.uniform_(*self.cfg.ranges.lin_vel_z)

...
class ComOffsetANDRollingAngularVelocityCommand(CommandTerm):
    r"""

    """

    cfg: ComOffsetANDRollingAngularVelocityCommandCfg
    """The configuration of the command generator."""

    def __init__(self, cfg: ComOffsetANDRollingAngularVelocityCommandCfg, env: ManagerBasedEnv):
        """Initialize the command generator.
        ...
        # crete buffers to store the command
        # -- command: offset, yaw vel
        self.vel_command_b = torch.zeros(self.num_envs, 2, device=self.device)
        #self.heading_target = torch.zeros(self.num_envs, device=self.device)
        self.is_heading_env = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device)
        self.is_standing_env = torch.zeros_like(self.is_heading_env)
        # -- metrics
        self.metrics["error_com_pos"] = torch.zeros(self.num_envs, device=self.device)
        self.metrics["error_ang_z"] = torch.zeros(self.num_envs, device=self.device)
    ...
    def _update_metrics(self):
        ...
        max_command_step = max_command_time / self._env.step_dt
        # com detection
        default_root_pos_state = self.robot.data.default_root_state[:, 0:3]
        current_root_pos_w = self.robot.data.root_link_pos_w
        current_root_com_pos_w = self.robot.data.root_com_pos_w
        # logs data
        self.metrics["error_com_pos"] += (
            torch.norm(self.vel_command_b[:, :2] - self.robot.data.root_lin_vel_w[:, :2], dim=-1) / max_command_step
        )
        self.metrics["error_ang_z"] += (
            (self.vel_command_b[:, -1] - self.robot.data.root_ang_vel_b[:, -1]) / max_command_step
    ...
    def _resample_command(self, env_ids: Sequence[int]):
        ...
        r = torch.empty(len(env_ids), device=self.device)
        # -- linear velocity - x direction
        self.vel_command_b[env_ids, 0] = r.uniform_(*self.cfg.ranges.com_offset_z)
        # -- linear velocity - y direction
        self.vel_command_b[env_ids, 1] = r.uniform_(*self.cfg.ranges.rolling_ang_vel)
